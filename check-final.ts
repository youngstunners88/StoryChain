import { createPublicClient, http, formatUnits } from 'viem';
import { base } from 'viem/chains';

const USDC = '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913' as `0x${string}`;
const WETH = '0x4200000000000000000000000000000000000006' as `0x${string}`;
const CLAWKED = '0xe54F44b6e1F5F5e7BB2222b21Ba5aAe9FFf2d812' as `0x${string}`;
const WALLET = '0x0089395dBced5DE83D65f13a38140F70777D56F0' as `0x${string}`;

const ERC20_ABI = [
  {
    inputs: [{ name: 'account', type: 'address' }],
    name: 'balanceOf',
    outputs: [{ name: '', type: 'uint256' }],
    stateMutability: 'view',
    type: 'function',
  },
] as const;

async function main() {
  const client = createPublicClient({
    chain: base,
    transport: http('https://mainnet.base.org'),
  });

  const ethBalance = await client.getBalance({ address: WALLET });
  console.log(`ETH Balance: ${formatUnits(ethBalance, 18)}`);

  const usdcBalance = await client.readContract({
    address: USDC,
    abi: ERC20_ABI,
    functionName: 'balanceOf',
    args: [WALLET],
  });
  console.log(`USDC Balance: ${formatUnits(usdcBalance as bigint, 6)}`);

  const wethBalance = await client.readContract({
    address: WETH,
    abi: ERC20_ABI,
    functionName: 'balanceOf',
    args: [WALLET],
  });
  console.log(`WETH Balance: ${formatUnits(wethBalance as bigint, 18)}`);

  const clawkedBalance = await client.readContract({
    address: CLAWKED,
    abi: ERC20_ABI,
    functionName: 'balanceOf',
    args: [WALLET],
  });
  console.log(`CLAWKED Balance: ${formatUnits(clawkedBalance as bigint, 18)}`);
  
  // Check CLAWKED decimals
  const clawkedDecimals = await client.readContract({
    address: CLAWKED,
    abi: [{
      inputs: [],
      name: 'decimals',
      outputs: [{ name: '', type: 'uint8' }],
      stateMutability: 'view',
      type: 'function',
    }] as const,
    functionName: 'decimals',
  });
  console.log(`CLAWKED Decimals: ${clawkedBalance}`);
  console.log(`CLAWKED Balance (raw): ${clawkedBalance}`);
  console.log(`CLAWKED Balance (with decimals ${clawkedDecimals}): ${formatUnits(clawkedBalance as bigint, Number(clawkedDecimals))}`);
}

main();
